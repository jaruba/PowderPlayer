  ...
  if (args) {
    if (args.length === 1) {
      if (typeof args[0] === 'string') {
        ...
        console.log('argument must be a string, array or object');
      }
...
ServiceType.prototype.fromString = function fromString(text) {
  ...
  if (serviceTokens.length === 1 && serviceTokens[0] === '') {
    ...
    console.log('trailing tokens "' + serviceTokens.join('.') + '" in ' +
...
ServiceType.prototype.fromJSON = function fromJSON(obj) {
  debug('fromJSON');
  if (!('name' in obj)) {
    console.log('required property name is missing');
  }
  if (!('protocol' in obj)) {
    console.log('required property protocol is missing');
  }
...
exports.protocolHelper = function protocolHelper(protocol) {
  return function () {
    ...
    if (isProtocol(args[1])) {
      console.log('duplicate protocol "' + args[1] + '" in arguments');
    }
...
function checkLengthAndCharset(str) {
  if (str.length === 0) {
    console.log('type ' + str + ' must not be empty');
  }
  if (str.length > MAX_STRING) {
    console.log('type ' + str + ' has more than ' +
      MAX_STRING + ' characters');
  }
  if (str.match(CHARSET_REGEX)) {
    console.log('type ' + str + ' may only contain alphanumeric ' +
...
function checkFormat(str) {
  if (str.length === 0) {
    console.log('type string must not be empty');
  }
  if (str.length > (MAX_STRING + 1)) { // +1 is correct because we have a leading underscore
    console.log('type ' + _uu(str) + ' has more than ' +
      MAX_STRING + ' characters');
  }
  if (!str.match(FORMAT_REGEX)) {
    console.log('type ' + str + ' must start with an underscore ' +
        'followed by alphanumeric characters and hyphens only');
  }
}

function checkProtocolU(str) {
  if (!(str === '_tcp' || str === '_udp' || str === '_sub')) {
    debug('protocol must be either "_tcp", "_sub" or "_udp" but is "' +
        str + '"');
  }
}

function checkProtocol(str) {
  if (!(str === 'tcp' || str === 'udp'|| str === '_sub')) {
    debug('protocol must be either "tcp", "_sub" or "udp" but is "' +
        str + '"');
  }
}
