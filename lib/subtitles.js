			...
			while (!perfect) {
				...
				tempArr.forEach(function(ml,ij) {
					if (ij > 0) {
						...
						if (curVote > prevVote) {
							tempArr[ij] = tempArr[ij-1];
							tempArr[ij-1] = ml;
							perfect = false;
						}
					}
				});
				// reset subtitle names
				tempArr.forEach(function(ml,ij) {
					var reverter = ml.split(": ");
					reverter[0] = reverter[0].replace('"','').replace('"','');
					if (reverter[0].indexOf(" ") > -1) reverter[0] = reverter[0].split(" ")[0];
					if (ij == 0) reverter[0] = '"'+reverter[0]+'"';
					else reverter[0] = '"'+reverter[0]+" "+(ij+1)+'"';
					tempArr[ij] = reverter.join(": ");
				});
			}
