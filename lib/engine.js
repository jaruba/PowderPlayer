		...
		fs.exists(gui.App.dataPath+'\\vlc_playlist.m3u', function(exists) {
			if (exists) fs.unlink(gui.App.dataPath+'\\vlc_playlist.m3u', function() {
				fs.writeFile(gui.App.dataPath+'\\vlc_playlist.m3u', newM3U, function() {
					require('child_process').exec('"'+require('path').dirname(process.execPath)+'\\node_modules\\wcjs-player\\node_modules\\wcjs-renderer\\node_modules\\webchimera.js\\build\\Release\\vlc.exe" "'+gui.App.dataPath+'\\vlc_playlist.m3u"');
				});
			});
	...
	if ($('#main').css("display") == "table") {
		if (torLink.toLowerCase().replace(".torrent","") != torLink.toLowerCase()) {
			...
			if (torLink.substr(0,4) != "http" && torLink.substr(0,8) != "file:///") {
				...
				if (!isWin) {
					powGlobals.videos[thisVideoId].path = torLink.replace("file:///","");
				} else {
