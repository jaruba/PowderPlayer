function checkDownloaded(piece) {
	...
	powGlobals.files.forEach(function(el,ij) {
		if (piece >= el.firstPiece && piece <= el.lastPiece && piece > 0) {
			...
			if (ij == powGlobals.videos[wjs().currentItem()].index && wjs().state() == "opening") {
				if (firstTime == 0) {
					tempPrebuf = Math.floor((el.downloaded / (el.lastPiece - el.firstPiece)) *100*45);
					if (tempPrebuf <= 100 && tempPrebuf > prebuf) {
						prebuf = tempPrebuf;
						if (playerLoaded) wjs().setOpeningText("Prebuffering "+prebuf+"%");
					}
				}
			}
...
function announceNoPeers() {
	if (wjs().currentItem() == 0 && wjs().state() == "opening") {
