...
//			console.log("dlna started 1");
			if (lastDlnaSecond > 0) {
				if (lastDlnaSecond > 10) dlnaControls.seek(lastDlnaSecond);
				lastDlnaSecond = 0;
			}
		}
...
function findDlnaClient() {
    ...
    client.on('response', function (headers, statusCode, rinfo) { if (headers["LOCATION"]) dlnaClients.push(headers["LOCATION"]); });
	
	setTimeout(function() { checkClients(); },500);
